...
def scrape_vysledky_obce(url):
    """
    Načte výsledky voleb pro jednu obec.
    Pokud se nenajdou, hodnoty se zapíší jako '0'.
    """
    response = requests.get(url)
    response.raise_for_status()
    soup = BeautifulSoup(response.text, 'html.parser')
    ...
    for table in tables:
        for row in table.find_all('tr'):
            ...
            if len(cols) >= 2:
                ...
                if "Voliči v seznamu" in label:
                    volici_v_seznamu = value or '0'
                elif "Vydané obálky" in label:
                    vydane_obalky = value or '0'
                elif "Platné hlasy" in label:
                    platne_hlasy = value or '0'
    ...
    for table in tables:
        for row in table.find_all('tr')[1:]:
            cols = row.find_all('td')
            if len(cols) >= 3:
                nazev_strany = cols[1].get_text(strip=True)
                hlasy = cols[2].get_text(strip=True).replace('\xa0', '')
                hlasy_strany[nazev_strany] = hlasy or '0'
...
def uloz_do_csv(soubor, data, strany):
    ...
    """
    with open(soubor, mode='w', newline='', encoding='utf-8-sig') as f:
